...
En lugar de subir los archivos manualmente a un servidor cada vez que hago un cambio en mi código, he creado un "robot" (un flujo de trabajo o *workflow* en GitHub Actions) que lo hace por mí de forma automática. De esta manera, cada vez que modifico el archivo `index.html` y subo los cambios a GitHub (push), el robot detecta la modificación y publica la nueva versión de mi web en internet automáticamente.

## ⚙️ Cómo he configurado mi repositorio para GitHub Actions

Para que GitHub Actions funcione y pueda publicar mi web en GitHub Pages, he preparado el repositorio siguiendo dos pasos muy concretos:

### 1. La estructura de carpetas
GitHub **solo** buscará instrucciones de automatización si están guardadas en una carpeta muy específica que debe estar obligatoriamente en la **raíz de mi proyecto**. He creado esta ruta exacta:
```text
.github/workflows/
```
...
1. Fui a la página de mi repositorio en GitHub.com.
2. Hice clic en la pestaña superior llamada **Settings** (Configuración).
3. En el menú de la izquierda, busqué el apartado **Pages**.
4. Donde dice **Build and deployment**, abrí el desplegable "Source" (Origen) y seleccioné **GitHub Actions** en lugar de *Deploy from a branch*.
...
- **¿Cuándo se ejecuta? (`on: push`)**: Le indico al robot que esté "dormido" hasta que detecte que he hecho un `push` a la rama `main`, pero con una restricción especial (`paths:`): **solo** debe despertarse si el archivo modificado ha sido mi `index.html`.
- **¿Qué permisos necesita? (`permissions`)**: Para evitar hackeos o problemas de seguridad, los robots de Actions nacen sin poder tocar casi nada. Tengo que darle permiso explícito de escritura para que le dejen publicar cosas en el servicio de Pages (`pages: write`).
